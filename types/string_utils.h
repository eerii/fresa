...
namespace fresa
{
    ...
    [[nodiscard]] constexpr str_view lower(str_view s) {
        char* l = new char[s.size()]; // new c++20 dynamic allocation in constexpr
        std::transform(s.begin(), s.end(), l, [](char c) { return std::tolower(c); });
        s = str_view{l, s.size()};
        return s;
    }
    [[nodiscard]] constexpr str_view operator""_lower(const char* text, std::size_t size) {
        return lower(str_view{text, size});
    }

    //* split
    [[nodiscard]] constexpr auto split(str_view s, char del = ' ') {

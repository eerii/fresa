        ...
        for (const auto &s_id : subpass_list) {
            ...
            for (const auto &shader_id : subpass_shaders) {
                ...
                if (shaders.types.at(shader_id) == SHADER_DRAW) {
                    ...
                    //: Index buffer
                    VkIndexType index_type = meshes.index.stride == 2 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32;
                    vkCmdBindIndexBuffer(cmd, meshes.index.buffer.buffer, VkDeviceSize{0}, index_type);
                    
                    //: Draw indirect
                    for (auto &d_cmd : draw_objects)
                        vkCmdDrawIndexedIndirect(cmd, draw_commands.buffer.buffer, draw_commands.blocks.at(d_cmd).offset * sizeof(IDrawIndexedIndirectCommand), 1, sizeof(IDrawIndexedIndirectCommand));
                }
